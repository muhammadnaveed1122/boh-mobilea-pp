...
import { type CompareItem, useCompareStore } from '../store/compare.store';

function Thumb({ item, onRemove }: Readonly<{ item: CompareItem; onRemove: () => void }>) {
  return (
 ...
 * project thumbnails (each removable), a Clear action, and a primary
    ...
    >
      <View className="flex-row items-center justify-between gap-3">
        ...
        <View className="items-end gap-1.5">
          ...
          <Pressable
            disabled={!canCompare}
            onPress={() => router.push('/project-management/compare')}
            accessibilityRole="button"
            accessibilityLabel={`Compare ${items.length} projects`}
            className={cn(
