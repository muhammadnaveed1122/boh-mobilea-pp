...
import type { PublicProjectsFilters } from '../types';

interface Props {
  visible: boolean;
  initialFilters: PublicProjectsFilters;
  onClose: () => void;
  onApply: (next: PublicProjectsFilters) => void;
}
...
export function ProjectFiltersSheet({
  ...
  const [draft, setDraft] = useState<PublicProjectsFilters>(initialFilters);
  const { data: states = [] } = useStates();

  const stateOptions: FilterOption[] = [
    { value: 'all', label: 'All States' },
    ...states.map((s) => ({ value: s.id, label: s.city })),
  ];

  const reset = () => {
    setDraft({ search: initialFilters.search, status: 'all' });
  };
        ...
        >
          <Section title="Availability">
            <SingleSelect
              ...
              onChange={(v) =>
                setDraft({ ...draft, status: (v as PublicProjectsFilters['status']) ?? 'all' })
              }
            />
          </Section>
          ...
          <Section title="Handover">
            <SingleSelect
              ...
              value={draft.handover ?? 'all'}
              onChange={(v) => setDraft({ ...draft, handover: v })}
            />
          </Section>
        </ScrollView>
